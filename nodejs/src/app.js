...
function permaLinks(idArray) {
  return new Promise( async (res, rej) =>  {
    let retArray = [];
    for (const id of idArray) {
      try {
        const response = await axios.get(`https://graph.instagram.com/v18.0/${id}?fields=permalink&access_token=${process.env.TEST_USER_TOKEN}`);
        retArray.push(response.data.permalink);
      } catch (err) {
        console.log(err);
      }
    }
